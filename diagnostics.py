...
def state_db_path_for_engine(engine: Any) -> Path:
    ...
    # Fork: default must resolve under the active HERMES_HOME, not the raw real
    # home — a bare Path.home()/".hermes" derives a PROD state.db path under
    # hermetic runs (same freeze class as the 2026-07-24 incident / t_43d5c42d).
    # The host module is absent outside the agent process (e.g. the standalone
    # test suite), so fall back to the LCM db's own directory rather than
    # failing the whole diagnostic.
    store_db_path = getattr(engine._store, "db_path", None)
    if store_db_path is None:
        try:
            from hermes_constants import get_hermes_home

            store_db_path = get_hermes_home() / "lcm.db"
        except ImportError:
            store_db_path = Path(engine._config.database_path)

    db_path = Path(store_db_path)
    return _enforce_state_db_containment(
