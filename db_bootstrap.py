...
#
# Fork note: the replay-dedup + timestamp-fidelity columns (superseded_by,
# timestamp_*) are ALSO decoupled from this numeric counter — they run
# unconditionally+idempotently via ``ensure_messages_dedup_columns`` under the
# named ``messages_dedup_v1`` marker, so they don't collide with upstream's
# numeric v5 (conversation_id). See run_versioned_migrations.
SCHEMA_VERSION = 5
...
_V5_CORE_OPTIONAL_COLUMNS: dict[str, frozenset[str]] = {
    "messages": frozenset({
        ...
        # Fork sidecars: plaintext search index (encryption) + replay-dedup /
        # timestamp-fidelity columns. All additive, backward-compatible.
        "search_content", "superseded_by",
        "timestamp_original", "timestamp_reconstructed", "timestamp_verified",
    }),
}
...
def run_versioned_migrations(conn: sqlite3.Connection) -> None:
    ...
    # Upstream numeric v5 = message conversation_id.
    ...
    # Fork: replay-dedup + timestamp-fidelity columns. Decoupled from the numeric
    # counter (upstream took numeric v5 for conversation_id) — run unconditionally
