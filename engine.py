...
class LCMEngine(CompactionMixin, ResetStateMixin, ReconcileMixin, AuxiliarySessionMixin, PlaceholderLedgerMixin, BypassMixin, ContextEngine):
    ...
    def __init__(self, config: LCMConfig | None = None,
        ...
        # Fork P2 calibration knobs (read by the ContextEngine ABC calibration
        # methods via getattr). Sourced ONCE here from this engine's own config so
        # the process-global singleton is never mutated per-agent (Greptile PR #111).
        ...
        # Fork: leaf-pass count of the most recent compress() that reached assembly;
    ...
    def _resolve_db_path(self, hermes_home: str = "") -> Path:
        ...
        try:
            ...
            # Host module absent (standalone/test context): fall back to the
            # conventional home rather than failing construction.
    ...
    def _bind_storage(self, db_path: str | Path, hermes_home: str = "") -> None:
        ...
        try:
            ...
            # Fork: SummaryDAG needs ingest_protection_config for summary-node redaction.
    ...
    def update_from_response(self, usage: Dict[str, Any]) -> None:
        ...
        # Fork: pair the skew (P2 "compact on the truth" calibration) — the real
    ...
    def _reset_session_counters(self) -> None:
        ...
        Fork override: also clears the fail-open resilience counters/state.
        """
        super()._reset_session_counters()
        self._last_condensations = []
        self._degraded = False
        self._last_degraded_reason = ""
        self._last_fail_open_at = 0.0
    ...
    def _reset_session_scoped_runtime_state(self) -> None:
        ...
        Fork override: after the mixin resets counters/compaction-progress/
        placeholder-digest state, also clear the P2 skew calibration. Skew is
        per-conversation; clear it at a real session boundary so a ratio learned
        in one conversation can't scale down a fresh session's first preflight
        (Greptile #111). NOT called on the compression boundary path (same
        conversation continuing → skew is still valid there).
        """
        super()._reset_session_scoped_runtime_state()
    ...
    def _ingest_messages(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # Fork: when the cursor reconcile advances PAST durable (non-scaffold) rows
    ...
    def _sanitize_active_context_messages(
        ...
        *,
        ...
        # NOT done when this sanitize feeds ASSEMBLY BUDGET SELECTION: merging
        # first glues a small, high-value row (e.g. the latest compact status)
        # onto a bulky one, so the budget check evicts BOTH as a single
        # oversized unit and `_is_budget_droppable_tail_message` loses the
        # granularity it needs. The final emitted context is merged separately.
    ...
    def _latest_user_context_anchor(
        ...
        for message in reversed(messages):
            ...
            if (
                ...
                continue
            # UPSTREAM 0.21 RULE (supersedes the fork's dedupe-comparison):
            # a preserved-objective scaffold is DERIVED context, never a real
            # user turn, so on reaching one we stop — it must never be promoted
            # as the next anchor. This is strictly more conservative than the
            # fork's old behavior (which returned the SANITIZED scaffold as a
            # fresh anchor and deduped it against the selected tail): you cannot
            # duplicate what you never promote. It also closes a re-injection
            # hole — re-anchoring replayed the carried block's untrusted
            # `active_memory` body back into the active context.
