...
class LCMConfig:
    ...
    # Fork subsystem (backs _dynamic_fresh_tail_count/_refresh_fresh_tail_token_budget);
    # supersedes upstream's minimal ``fresh_tail_max_tokens: int = 0`` suffix cap.
    ...
    def from_env(cls) -> "LCMConfig":
        ...
        # Fork-style short aliases for the module-level env parsers. Upstream
        # dropped the fork's from_env-local _int/_float/_str closures in favor of
        # the ENV_FIELD_SPECS loop, but the fork's preserved manual reads (calibration
        # knobs, encryption/retention) still call them, so bind the aliases here.
        _int = _parse_int_env
        ...
        # Fork: token-budgeted fresh-tail subsystem env reads (fork-only helpers
        # _lcm_config_bool / _hermes_lcm_int / _hermes_compression_float). Kept in
        # the fork's style; not source-tracked (fork subsystem predates provenance).
        ...
        # Fork-only calibration knobs (NOT in ENV_FIELD_SPECS): env override ->
        # compression.<key> config-file fallback -> default. The uniform loop
        # cannot replicate the config-file fallback, so keep these explicit.
        c.skew_floor = _float(
            "LCM_SKEW_FLOOR",
        ...
        # Fork-only fields (NOT in ENV_FIELD_SPECS; tie to storage_security.py).
        # The uniform loop cannot read them, so keep explicit. summary_circuit_breaker_*,
        # expansion_*, summary_timeout_ms, database_path, new_session_retain_depth and
        # doctor_clean_apply_enabled are spec-covered and handled by the loop.
